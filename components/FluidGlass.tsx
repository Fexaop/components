...
export default function FluidGlass({ mode = 'lens', lensProps = {}, barProps = {}, cubeProps = {} }: FluidGlassProps) {
    ...
    // Set default fluid properties for a more widespread effect
    const defaultLensProps = {
        intensity: 4.0,
        distortion: 1.0,
        radius: 0.2,
        curl: 20,
    ...
    const rawOverrides = mode === 'bar' ? barProps : mode === 'cube' ? cubeProps : { ...defaultLensProps, ...lensProps };
...
function Lens({ modeProps, children, ...p }: { modeProps?: ModeProps; children?: ReactNode } & MeshProps) {
    ...
    const cleanTarget = useFBO();

    useFrame((state) => {
        state.gl.setRenderTarget(cleanTarget);
