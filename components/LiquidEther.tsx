...
`;
    const color_frag = `
		...
		void main(){
			...
			float line = smoothstep(0.03, 0.04, lenv) - smoothstep(0.05, 0.06, lenv);
			gl_FragColor = vec4(1.0, 1.0, 1.0, line * 0.8);
		}
`;
...
`;
    const externalForce_frag = `
		...
		float d = 1.0 - min(length(circle) * 0.5, 1.0);
		d = smoothstep(0.0, 1.0, d);
		
		// Clean front mask - only show front semicircle
		vec2 forceDir = normalize(force + vec2(0.001, 0.001));
		float frontDot = dot(circle, forceDir);
		float frontMask = smoothstep(-0.1, 0.1, frontDot);
		
		gl_FragColor = vec4(force * d * frontMask, 0.0, 1.0);
}
`;
    const poisson_frag = `
		precision highp float;
...
`;
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(...args: any[]) {
        const props = args[0] || {};
        const mouseSpeed = Mouse.diff.length();
        let forceX = 0;
        let forceY = 0;

        if (mouseSpeed > (props.mouse_force_threshold || 0)) {
          forceX = (Mouse.diff.x / 2) * (props.mouse_force || 0);
          forceY = (Mouse.diff.y / 2) * (props.mouse_force || 0);
        }
    ...
    class Simulation {
      ...
      update() {
        ...
        this.externalForce.update({
          ...
          mouse_force_threshold: this.options.mouse_force_threshold,
          cellScale: this.cellScale
        });
    ...
    const applyOptionsFromProps = () => {
      ...
      Object.assign(sim.options, {
        ...
        mouse_force_threshold: mouseForceThreshold,
        cursorSize,
        isViscous,
        viscous,
